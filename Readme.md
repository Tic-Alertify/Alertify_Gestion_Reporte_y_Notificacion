...
```bash
git clone <https://github.com/Tic-Alertify/Alertify\\\_Gestion\\\_Reporte\\\_y\\\_Notificacion.git>
cd programa
```
...
DB\\\_PORT=1433
DB\\\_USERNAME=sa
DB\\\_PASSWORD=Alertify\\\_Pass2026!
DB\\\_DATABASE=AlertifyDB
...
JWT\\\_SECRET=tu\\\_secret\\\_key\\\_aqui

# Firebase (Opcional, necesario para notificaciones)
FIREBASE\\\_PROJECT\\\_ID=alertify-xxx
...
* **SQL Server 2022** en puerto `1433` con usuario `sa` / contraseña `Alertify\\\_Pass2026!`
...
**Tabla: REPORTES\_CIUDADANOS**

```sql
CREATE TABLE REPORTES\\\_CIUDADANOS (
...
FROM HISTORIAL\\\_OFICIAL\\\_INCIDENTES 
...
Validaciones Previas (Geofencing)
    ↓
Guardado en SQL Server (REPORTES\\\_CIUDADANOS)
    ↓
...
### **1. Requisitos y Configuración del Frontend (Alertify\_App)**
...
&#x20;   `properties MAPS\\\_API\\\_KEY=AIzaSyA\\\_tu\\\_clave\\\_de\\\_google\\\_maps\\\_aqui `
...
&#x20;   `kotlin // core/build.gradle.kts (Ejemplo de configuración) buildTypes { debug { // Reemplaza 192.168.100.35 con tu IPv4 de Windows (ipconfig) buildConfigField("String", "API\\\_BASE\\\_URL", "\\\\"http://192.168.100.35:3000/\\\\"") buildConfigField("String", "SOCKET\\\_URL", "\\\\"http://192.168.100.35:3000\\\\"") } } `
...
&#x20;   `typescript await app.listen(3000, '0.0.0.0'); // Fundamental para pruebas en dispositivo real `
...
User: sa
Password: Alertify\\\_Pass2026!
Database: AlertifyDB

# O desde terminal (si tienes sqlcmd)
sqlcmd -S localhost,1433 -U sa -P "Alertify\\\_Pass2026!"
```
...
FROM REPORTES\\\_CIUDADANOS
ORDER BY FechaHoraRegistro DESC;

-- Reportes validados
SELECT \\\* FROM REPORTES\\\_CIUDADANOS WHERE Estado = 1;
